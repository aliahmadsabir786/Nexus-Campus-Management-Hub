...
async function confirmDelStudent(sid){const s=students.find(x=>x.id===sid);if(!await confirmAction({title:'Delete student',message:`Delete ${s?s.name:'this student'}?`,note:'Attendance, grades and fee records for this student are removed too. This cannot be undone.',confirmLabel:'Delete student'}))return;students=students.filter(x=>x.id!==sid);if(modalState==="viewStudent"||modalState==="editStudent"){closeModal();}refreshContent();showToast('success','Student deleted.');}
function changeStudentPhoto(sid,input){const file=input.files[0];if(!file)return;if(file.size>2*1024*1024){showToast('warning','That photo is over 2 MB. Please choose a smaller image.');return;}const reader=new FileReader();reader.onload=async ev=>{const s=students.find(x=>x.id===sid);if(s){s.photo=ev.target.result;formData={...s};}render();try{const res=await fetch(`/api/students/${sid}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({photo:ev.target.result})});const d=await res.json();if(!d.success)notify.fromError(d,'Could not save the new photo.');else showToast('success','Photo updated.');}catch(e){console.error('Photo update error:',e);showToast('error','Could not reach the server to save the photo.');}};reader.readAsDataURL(file);}
...
async function revertFee(sid){if(!await confirmAction({title:'Revert fee status',tone:'warning',icon:'↺',message:'Set this fee back to Pending?',note:'The recorded payment date is cleared.',confirmLabel:'Revert to pending'}))return;const s=students.find(x=>x.id===sid);if(s){s.feeStatus="pending";const v=(feeVouchers[s.id]||[])[0];if(v){v.status="pending";v.paidDate=null;}refreshContent();showToast('success','Fee reverted to pending.');}}
...
function submitAssignment(assignmentId,studentId,studentName,cls,input){const file=input.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{const newSub={id:"SUB"+Date.now(),assignmentId,studentId,studentName,cls,fileData:ev.target.result,fileName:file.name,submittedAt:today,grade:null,feedback:"",status:"submitted"};window["sub_"+newSub.id+"_data"]=ev.target.result;submissions.push(newSub);refreshContent();showToast('success','Assignment submitted.');};reader.readAsDataURL(file);}
function submitGrade(){const subId=formData.subId;const grade=parseInt(document.getElementById("f-grade")?.value||formData.grade||"0");const feedback=document.getElementById("f-feedback")?.value||formData.feedback||"";if(isNaN(grade)||grade<0||grade>100){showToast('warning','Enter a grade between 0 and 100.');return;}const sub=submissions.find(s=>s.id===subId);if(sub){sub.grade=grade;sub.feedback=feedback;sub.status="graded";}closeModal();refreshContent();showToast('success','Grade saved.');}
function submitCreateAssignment(){const title=(document.getElementById("f-title")?.value||formData.title||"").trim();if(!title){showToast('warning','Enter an assignment title.');return;}const teacher=teachers.find(x=>x.id===currentUser?.id);const dueDate=document.getElementById("f-dueDate")?.value||formData.dueDate||"";if(!dueDate){showToast('warning','Set a due date.');return;}const newA={id:"A"+Date.now(),title,subject:document.getElementById("f-subject")?.value||formData.subject||SUBJECTS[0],cls:document.getElementById("f-cls")?.value||formData.cls||"CS-A",teacherId:currentUser.id,teacherName:teacher?.name||"Teacher",dueDate,description:document.getElementById("f-description")?.value||formData.description||"",createdAt:today};assignments.push(newA);closeModal();refreshContent();showToast('success',`Assignment "${title}" created.`);}
